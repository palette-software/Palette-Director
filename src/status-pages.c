...
enum {
	kNO_MAPPING_AS_PRIORITY = -123456
};
...
static void status_table_cell(request_rec* r, const int prio) {
    ...
    if (prio != kNO_MAPPING_AS_PRIORITY) {
		ap_rprintf(r, "<span title='Active' class='tb-icon-process-status tb-icon-process-status-active'><small style='margin-left: 25px;'><em>priority: </em><b>%d</b></small></span>", prio);
    }
...
static void find_all_hosts(const binding_rows* b, key_extractor_fn extractor_fn, const char** output, size_t* output_count) {
	size_t i;
...
static int mapping_priority_for(const binding_rows* b, const char* site_name, const char* worker_host) {
	size_t i, len = b->count;
	for (i = 0; i < len; ++i){
		const binding_row br = b->entries[i];
		if (strcmp(br.site_name, site_name) == 0 && strcmp(br.worker_host, worker_host) == 0) {
			return br.priority;
		}
	}
...
void status_page_html(request_rec* r, const binding_rows* b, const int add_style) {
    ...
    // list the sites as header
    {
        ...
        for(i=0; i < host_buf_size; ++i) {
			...
			ap_rprintf(r, "<th class='tb-data-grid-headers-line tb-data-grid-headers-line-multiline'><div class='tb-data-grid-header-text'>%s</div></th>", host_buf[i]);
        }
    ...
    // ----------------
    {
		...
		for (row=0; row < sites_buf_size; ++row) {
            ap_rprintf(r, "<tr>");
			ap_rprintf(r, "<td class='tb-data-grid-separator-row'><span class='tb-data-grid-cell-text tb-lr-padded-wide ng-scope'>%s</span></td>", sites_buf[row]);

			for (col=0; col < host_buf_size; ++col) {				
				status_table_cell(r, mapping_priority_for(b, sites_buf[row], host_buf[col]) );
			}
